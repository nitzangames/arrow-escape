...
- A grid board, **completely filled**: every playable cell belongs to exactly one snake. A **piece is a snake**: an ordered path of connected cells with bends (up to 16 at late-game wave peaks — see §Difficulty waves), ending in an arrowhead that points away from the body, colinear with the path's end segment (like the reference game). Length-1 pieces (dots) are allowed as packing filler.
...
- **Loading:** boot fetches `levels.json.gz` and inflates it with the built-in `DecompressionStream('gzip')` into `gd.baked` — **off the critical path** (fired after the menu is interactive), so first paint never waits on the ~1 MB download. Until it lands, `getLevel` falls back to generation; any load failure (missing file, no `DecompressionStream`) leaves `gd.baked = null` permanently and the game stays on pure runtime generation — the data file is an optimization, never a hard dependency.
