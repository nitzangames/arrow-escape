...
Post-review board-quality round: per-attempt shuffled tiling seeds (dead cells still first) plus randomized end choice in the peel fixed a ~3:1 up/left arrowhead skew (scan-order seeding pinned every path's seed end to the top-left frontier); a singles-merge pass (absorb dots into adjacent path ends when the merge stays statically alive) cut shaped-board dot ratios from ~38% to ~21% median; generateLevel now throws explicitly if a candidate pool ever empties instead of crashing on undefined.
